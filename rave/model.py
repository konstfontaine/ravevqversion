...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        return loss, quantized.transpose(1, 2).contiguous(), perplexity, encodings
...
class RAVE(pl.LightningModule):
    def __init__(self,
        ...
        # implement this if latent size (manual configs) != 128
        # vq_embedding_dim = encoder_out_size 
        ...
        #######Pre Vector Quantizer Convolution##############
        self.pre_vq_conv = cc.Conv1d(
            encoder_out_size, 
            vq_embedding_dim, 
            kernel_size = 1, 
            stride = 1)
    ...
    def reparametrize(self, mean, scale):
        ...
        loss_vq, quantized, perplexity, encodings = self.vq(z)

        return loss_vq, quantized, perplexity, encodings, kl
    ...
    def training_step(self, batch, batch_idx):
        ...
        #loss_vq, quantized, perplexity, encodings, _ = self.reparametrize(*self.encoder(x)) 
        
        # pass kl here as well in order to log later in logs
        loss_vq, quantized, perplexity, encodings, kl = self.reparametrize(*self.encoder(x)) 
        p.tick("encode")
        ...
        # DECODE LATENT
        y = self.decoder(quantized, add_noise=self.warmed_up)
        p.tick("decode")
        ...
        loss_gen = distance + loss_adv + beta * loss_vq
    ...
    ######## implementation of vector quantization in the encode function ######
    def encode(self, x):
        if self.pqmf is not None:
            x = self.pqmf(x)

        mean, scale = self.encoder(x)
        loss_vq, z, perplexity, encodings, kl = self.reparametrize(mean, scale)
        return z 
    ...
    def validation_step(self, batch, batch_idx):
        ...
        mean, scale = self.encoder(x)
        _, z, _, _, kl = self.reparametrize(mean, scale) #z == quantized here
